          ...
          <div key={it.id} className="rounded-md border bg-white p-3 dark:bg-slate-900/70">
            <div className="flex items-center justify-between">
              <div>
                ...
                {it.dueDate && (
                  <div className="text-xs text-muted-foreground">Due {it.dueDate}</div>
                )}
              </div>
              <div className="flex gap-2">
                <select
                  ...
                  onChange={(e) =>
                    platformActions.setMoveOpStatus(
                      it.id,
                      e.target.value as PlatformMoveOp["status"]
                    )
                  }
                >
                ...
                <Button variant="ghost" onClick={() => platformActions.deleteMoveOp(it.id)}>
                  Delete
                </Button>
              </div>
            </div>
