...
export default function StudyPromptButton({ topicName, template, context }: Props) {
  ...
  const ctx =
    context ??
    "Australian Christian husband/dad, health + routines focus, wants practical steps and Scripture where relevant.";
  ...
  return (
    <button
      onClick={handleCopy}
      className="text-sm px-3 py-1 border rounded bg-slate-100 dark:bg-slate-700"
    >
      Copy Study Prompt
    </button>
  );
