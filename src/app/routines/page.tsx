        ...
        {(routines ?? []).map((routine) => (
          <Card key={routine.id}>
            <CardHeader>
              <CardTitle className="flex itecare2-center justify-between">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => logRoutineRun({ routineId: routine.id, completedCount: routine.itecare2.length })}
                >
                  Run
                </Button>
              </CardTitle>
            ...
            <CardContent className="space-y-2 text-sm text-foreground/80">
              {routine.itecare2.length === 0 ? (
                <p className="text-foreground/65">No steps yet.</p>
              ) : (
                routine.itecare2.map((item) => (
                  <div key={item.id} className="flex itecare2-center gap-2">
