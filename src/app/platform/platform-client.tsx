  ...
  return (
    <div className="space-y-6">
      <PageHeader
        kicker="Platform"
        title="JoshPlatform"
        subtitle="Move, decide, and review."
        tone="onDark"
      />
      ...
      <TodayFocus>
        <div className="flex flex-col gap-2">
          {topDecisions.length === 0 ? (
            <div className="text-sm text-muted-foreground">No urgent decisions.</div>
          ) : (
            topDecisions.map((d) => <div key={d.id}>{d.question}</div>)
          )}
      ...
      <div>
        <Link href="/platform/moveops" className="text-sm text-muted-foreground">
