...
const lanes = [
  ...
  },
  {
    ...
    tags: ["care2"],
    description: "Care coordination, treatment, and rest rhythcare2.",
  },
];
  ...
  return (
    <Card className="space-y-3">
      ...
      <CardContent className="space-y-3">
        ...
        <CareSection title="Appointments" itecare2={events.filter((ev) => intersects(ev.tags, lane.tags))} renderItem={(item) => <EventRow key={item.id} event={item} />} empty="No appointments yet." />
        <CareSection title="Goals / Actions" itecare2={tasks.filter((t) => intersects(t.tags, lane.tags))} renderItem={(item) => <TaskRow key={item.id} task={item} />} empty="No goals yet." />
        <CareSection title="Providers" itecare2={bookmarks.filter((b) => intersects(b.tags, lane.tags))} renderItem={(item) => <ProviderRow key={item.id} bookmark={item} />} empty="No providers saved." />
        <CareSection title="Notes" itecare2={notes.filter((n) => intersects(n.tags, lane.tags) || n.lifeAreaSlug === "family" || n.lifeAreaSlug === "health")} renderItem={(item) => <NoteRow key={item.id} note={item} />} empty="No notes yet." />
      </CardContent>
    </Card>
  );
}
...
function InfoRow({
  ...
  return (
    <div className="flex itecare2-start gap-3 rounded-lg border border-neutral-200/80 bg-white/80 px-3 py-2 text-card-foreground dark:border-slate-800 dark:bg-slate-900/60">
...
function CareSection<T>({ title, itecare2, empty, renderItem }: CareSectionProps<T>) {
  return (
    <div className="space-y-2 rounded-lg border border-slate-200/70 bg-white/50 p-3 dark:border-slate-800 dark:bg-slate-900/50">
      <div className="flex itecare2-center justify-between">
        ...
        <p className="text-xs text-card-foreground/60">{itecare2.length} item{itecare2.length === 1 ? "" : "s"}</p>
      </div>
      {itecare2.length === 0 ? <p className="text-sm text-card-foreground/70">{empty}</p> : <div className="space-y-2">{itecare2.map(renderItem)}</div>}
    </div>
  );
}
