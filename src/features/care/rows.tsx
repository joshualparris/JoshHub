    ...
    <div className={rowClass}>
      {editing ? (
        <EditorActions onSave={save} onCancel={() => setEditing(false)}>
          <Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Provider name" />
          <Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://... (optional)" />
        ...
        <RowDisplay
          ...
          lines={[bookmark.url || "No URL saved", bookmark.tags.length ? `Tags: ${bookmark.tags.join(", ")}` : ""]}
          onEdit={() => setEditing(true)}
          onDelete={() => deleteWithConfirmation("Delete this provider?", () => deleteBookmark(bookmark.id))}
        />
      )}
    </div>
  );
    ...
    <div className={rowClass}>
      {editing ? (
        ...
        <RowDisplay
          ...
          onEdit={() => setEditing(true)}
          onDelete={() => deleteWithConfirmation("Delete this appointment?", () => deleteEvent(event.id))}
        />
      )}
    </div>
  );
...
export function TaskRow({ task }: { task: Task }) {
  ...
  return (
    <div className={rowClass}>
      {editing ? (
        <EditorActions onSave={save} onCancel={() => setEditing(false)}>
          <Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Goal" />
          <div className="grid grid-cols-2 gap-2">
            <select className="rounded-md border border-border bg-background px-2 py-1 text-sm" value={priority} onChange={(event) => setPriority(event.target.value as Task["priority"])}>
        ...
        <RowDisplay
          title={task.title}
          lines={[`Priority: ${task.priority.toUpperCase()}${task.dueDate ? ` • Due ${task.dueDate}` : ""}`, task.tags.length ? `Tags: ${task.tags.join(", ")}` : ""]}
          onEdit={() => setEditing(true)}
          onDelete={() => deleteWithConfirmation("Delete this goal/action?", () => deleteTask(task.id))}
        />
      )}
    </div>
  );
    ...
    <div className={rowClass}>
      {editing ? (
        <EditorActions onSave={save} onCancel={() => setEditing(false)}>
          ...
          <Textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="Details" />
          <TagInput value={tags} onChange={setTags} />
        </EditorActions>
      ) : (
        <RowDisplay
...
function TagInput({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <Input value={value} onChange={(event) => onChange(event.target.value)} placeholder="tags, comma separated" />;
}

function EditorActions({ children, onSave, onCancel }: { children: React.ReactNode; onSave: () => Promise<void>; onCancel: () => void }) {
  return (
...
function RowDisplay({ title, lines, onEdit, onDelete }: { title: string; lines: string[]; onEdit: () => void; onDelete: () => void }) {
  return (
    <div className="flex items-start justify-between gap-3">
      <div className="space-y-1">
        <p className="font-medium">{title}</p>
        {lines.filter(Boolean).map((line) => <p key={line} className="text-sm text-muted-foreground">{line}</p>)}
      </div>
      <div className="flex gap-1">
        <Button size="icon" variant="ghost" onClick={onEdit} aria-label={`Edit ${title}`}><Pencil className="h-4 w-4" /></Button>
        <Button size="icon" variant="ghost" onClick={onDelete} aria-label={`Delete ${title}`}><Trash className="h-4 w-4" /></Button>
      </div>
    </div>
  );
}
