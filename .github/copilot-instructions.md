...
1. `codingprinciples.md` — canonical cross-repository engineering standard (v5.1); JoshHub is Tier 2.
2. `AGENTS.md` — standing repo-specific workflow, safety rules and checkpoint protocol.
3. `docs/PROJECT_STATE.md` — current repository state, completed checkpoints and exact next work.
4. `docs/code-audit/TRIAGE.md` — stabilisation order.
5. `docs/code-audit/CONFORMANCE.md` — repository-specific measurable conformance evidence.
6. the relevant report under `docs/code-audit/` for the area being changed.
...
If a task cannot satisfy a MUST in `codingprinciples.md`, if canonical authority is unclear, or if the affected system cannot be understood from available evidence, follow its blocker protocol instead of guessing.

- Project type: Next.js (App Router) + TypeScript + Tailwind CSS. App lives in `src/app`.
- Assurance tier: Tier 2 — durable personal software with persistent local data.
...
- Routing & structure:
  - Routes are under `src/app`. Top-level routes include `dashboard`, `apps`, `life`, `notes`, `tasks`, `capture`, `health`, `calendar`, `projects`, `routines`, `settings`, `family`, and `engineering-principles`.
...
- Scripts and developer workflows (run from repo root):
  ...
  - Tests: `npm test` (terminating `vitest run` in the current package scripts)
...
- Integration points & external dependencies:
  - Hosting: Vercel (Next.js App Router); many catalogue items point to GitHub Pages or itch.io — maintain links in `src/data/apps.ts`.
...
If guidance conflicts, apply `codingprinciples.md` first, then `AGENTS.md`, then `docs/PROJECT_STATE.md`, then the current code-audit documents. Record any architectural decision that changes repo-specific rules in the same checkpoint as the code change.
