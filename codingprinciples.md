...
Keep this file at the repository root as `codingprinciples.md`. Reference it from `AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`, or equivalent agent instructions.
...
Repository-specific architecture and stack conventions belong in that repository, not in the universal standard. Use `REPO_CONVENTIONS.md`, the README, or `docs/decisions/` for durable project-specific decisions.

Do not silently fork this standard inside one repository. Change the shared standard deliberately and increment its version.
...
Words such as “probably”, “ideally”, “where appropriate” and “generally” do not create extra requirement levels.
...
**Universal safety floor:** always apply §1; §2.1, §2.3–2.4; §3.4, §3.6–3.7; §5.1–5.2; §6.1, §6.3; §7.1; §8.4; §9.4 whenever a coding agent is used; and §10 for maintained repos.
...
**Tier 1:** universal safety floor. Valid only while the repo remains genuinely experimental. Promote to Tier 2 if it stores persistent data somebody would care about losing, becomes regularly used, or becomes a dependency of another maintained system.

**Tier 2:** Tier 1 plus §2.2 and §2.5; §3.1–3.3 and §3.5; all of §4; §5.3–5.4; §6.4; §7.2; §8.1–8.3 and §8.5; and full §10 documentation. Promote to Tier 3 if it handles money, health data, workplace/client information, authentication/authorisation, other people's private data or similarly high-consequence data.

**Tier 3:** the entire standard. Convenience never excuses a relevant MUST.

**LLM/agent overlay:** if a repo contains an LLM-driven feature, §9.1–9.3 apply regardless of tier. If a coding agent modifies any repo, §9.4 applies regardless of tier.
...
Copies elsewhere **MUST** have an explicit role: read-only mirror, archived predecessor, backup, deliberate fork or generated distribution copy. A mirror **MUST NOT** receive independent feature development. If two repos both claim to be the same active project, affected work **MUST stop until one is selected**.
...
If context is unavailable, authoritative instructions conflict, the task requires violating a MUST, recovery cannot be established, an irreversible action cannot be verified, required access/tooling is unavailable, the canonical repo is unclear, or materially different interpretations remain:
...
Before changing code, understand enough of the surrounding path to explain what it does, why it exists, callers, dependencies, dependants, input origins, output destinations, persistent state touched and assumptions that must remain true. Investigation depth follows consequence of failure.
...
Prefer `UI → application/features → domain/services → persistence/infrastructure`. Domain/persistence **MUST NOT** depend on presentation for convenience. Dependency cycles **SHOULD** be removed; use interfaces, ports, callbacks or injection where framework constraints invert control.
...
Work that can grow from external input, generated behaviour or repeated failure **MUST** have a sensible bound: retries, model/tool calls, uploads, response sizes, untrusted collections, external recursion, queues, network waits and context history. Fixed inherently bounded work needs no artificial limit.
...
Logs **MUST NOT** contain passwords, API keys, access/refresh tokens, auth secrets or complete sensitive payloads. Sensitive personal data **SHOULD NOT** be logged unless documented and appropriately protected. Diagnostic identifiers **SHOULD** be minimised, scoped and pseudonymised where practical. Prefer request IDs, error categories, build versions and bounded technical context.
...
User-facing software **MUST** work in environments the repo claims to support. Where relevant verify keyboard operation, assistive semantics, contrast, focus, reduced motion, touch targets, responsive layout and real loading/error states. The repo defines supported devices, browsers and input methods.
...
At the required assurance level, CI **MUST** run declared gates, commonly format/lint, static/type analysis, tests, build and dependency/security checks. Report each result accurately.

Do not say “tests passed” when only the build ran. Do not say “the app works” merely because CI is green.
...
Do not complicate code for speculative performance gains. Performance work **SHOULD** begin with a measurement demonstrating the problem and **SHOULD** record a reproducible before/after measurement. Correctness must not be traded for speed.
...
Code owns facts; the model proposes interpretation, language and possibilities. Money, HP, resources, inventory, XP, dice outcomes, permissions, scores and save state **MUST** be owned by deterministic application logic unless the product explicitly defines a different authority model. A model must not silently invent mechanical truth.
...
Maintained repositories **MUST** make discoverable, when relevant: what the project does, how to run it, how to test it, how to build it, its assurance tier, important architectural constraints, where persistent data lives, and how it deploys.

Documentation must change when the behaviour it describes changes. Delete or fix stale docs. A confident description of a system that no longer exists is worse than no documentation.

---

## Assurance tiers

Choose by **consequence of failure**, not repository size.

### Tier 1 — Experimental

Examples: throwaway prototypes, experiments, proof-of-concept tools, games with no important persistence.

Required: universal safety floor. The aim is to learn quickly without creating misleading or dangerous complexity.

### Tier 2 — Durable personal or family software

Examples: regularly used personal software, family apps, persistent games, productivity tools and long-lived hobby systems.

Required: Tier 1 plus clear canonical data ownership, traceable data paths, boundary validation, deliberate schema evolution, recovery for important persisted data, tests around destructive/persistence-critical behaviour, supported-device/accessibility verification, controlled builds and maintained documentation.
...
Required: the full standard.

Tier 3 does not require ceremony for its own sake. It requires evidence that high-consequence risks have actually been addressed.
...
- [ ] I understand the affected data and dependency path deeply enough for this repository's risk.
...
- [ ] Results are reported separately and accurately.
...
- [ ] The change does not silently destroy, duplicate, expose or misrepresent important data.
...
- [ ] I confirmed I am changing the canonical repository, or this repo's non-canonical role explicitly permits the change.
...
- [ ] I applied the MUST requirements for that tier and applicable overlays.
...
A good change does not merely work today. It leaves enough truth that the next human or AI agent can understand **what it does, why it exists, how it was verified, how it can fail, and how to change it without guessing**.
