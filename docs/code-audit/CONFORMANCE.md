...
**Verify:** `npm test` plus `npm run validate:apps` plus, once its known failures
are fixed, `node scripts/check-assets.js`.

**Now:** ⚠️ 3 of 7 listed invariants are currently tested/enforced. Baseline CI
run `34337559585` proved the test suite, build, lint, and catalogue validator can
all execute automatically on `main`.
