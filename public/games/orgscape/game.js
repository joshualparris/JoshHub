...
const game = {
  ...
  events: [
    // Faction effects: [{Students:+1,Staff:-1}, ...]
    // sample events
    {
  ...
  for (let y = 0; y < game.floorHeight; y++) {
    ...
    for (let x = 0; x < game.floorWidth; x++) {
      let cell = {
        ...
        event: Math.random()<0.14 ? getRandom(game.events): null
      };
      row.push(cell);
    }
    floor.push(row);
  }
  // Always spawn one event in a random position
  let ex = Math.floor(Math.random()*game.floorWidth);
  let ey = Math.floor(Math.random()*game.floorHeight);
  floor[ey][ex].event = getRandom(game.events);
