...
class Game:
    ...
    def talk(self, *args):
        ...
        for nid in self.current_room().npcs:
            n = self.npcs[nid]
            if n.name.lower() == who or nid == who:
                target_id = nid; break
        if not target_id:
            say("They aren’t here.")
            return
    ...
    def arrest(self, *args):
        ...
        for nid in self.current_room().npcs:
            n = self.npcs[nid]
            if n.name.lower() == " ".join(args).lower() or nid == " ".join(args).lower():
                target_id = nid; break
        if not target_id:
            say("They aren’t here.")
            return
    ...
    def run(self):
        ...
        while True:
            ...
            if cmd in ("quit", "exit"):
                ...
                self.arrest(*args)
            elif cmd == "use" and args and args[0].lower() == "console":
                self.use_console()
            elif cmd == "save":
                self.save()
            elif cmd == "load":
